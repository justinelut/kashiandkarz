...
import { createUser, getUser } from "@/lib/appwrite";
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
	...
	const verifyEmailToken = useCallback(async (userId: string, token: string) => {
		try {
	...
	const loginWithOAuth = useCallback(async (provider: OAuthProvider) => {
		...
		try {
			...
			const errorMessage = error.message || `OAuth login failed with ${provider}`;
			setError(errorMessage);
			toast.error(errorMessage);
			throw error;
		} finally {
	...
	const handleOAuthCallback = useCallback(
		async (result: { secret: string; userId: string }) => {
			try {
				...
				const existingUser = await getUser(userData?.$id as string);
